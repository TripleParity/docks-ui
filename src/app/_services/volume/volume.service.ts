...
export class VolumeService {
    ...
    public createVolume(inputs: JSON): Observable<Volume> {
        return this.http.post(this.config.getAPIHostname() + '/docker/services/create',
    ...
    public inspectVolumes(id: string): Observable<Volume> {
        return this.http.get(this.config.getAPIHostname() + '/docker/networks/' + id, {responseType: 'json'})
            .pipe(map(x => {
                    return <Volume>x;
                }), catchError((err: HttpErrorResponse) => {
                    return ErrorObservable.create(<VolumeError>err.status);
    ...
    public deleteVolume(id: string, force: boolean): Observable<JSON> {
        ...
        return this.http.delete(this.config.getAPIHostname() + '/docker/networks/' + id, {params: params, responseType: 'json'})
            .pipe(map(x => {
                    return x;
                }), catchError((err: HttpErrorResponse) => {
                    return ErrorObservable.create(<VolumeError>err.status);
